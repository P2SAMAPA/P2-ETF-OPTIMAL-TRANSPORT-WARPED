...
st.set_page_config(page_title="Optimal Transport for Regime‑Warped Time Series", layout="wide")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'optimal_transport_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Warping slope: {etf['ot_score_norm']:.3f}</p>
...
with tab1:
    st.header("📐 Top ETFs by Optimal Transport Warping Slope (Auto Best Window)")
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the OT warping slopes.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
